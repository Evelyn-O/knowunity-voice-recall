 ...
 *
 * Lives at `/entry`, not `/` — the app's root now shows the exam-plan
 * path view (see (recall)/page.tsx's own doc comment) so a fresh visit
 * lands there first, with this fork still reachable via the normal
 * path → quiz → fork flow. Every internal "first-time entry fork"
 * reference (Try voice, getEntryForkRoute, etc.) points at `/entry`.
 */
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col items-center overflow-y-auto px-5 pt-[42px]">
          <div className="relative flex flex-col items-center">
            <div
              aria-hidden
              className="mascot-glow mascot-radial-glow absolute left-1/2 top-1/2 h-[270px] w-[270px]"
            />
            <div
              aria-hidden
