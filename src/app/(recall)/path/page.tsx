 ...
 * and, from there, the existing Voice Recall entry fork. No TopBar (this
 * screen has no exit X, no progress bar, no streak chip in its own Figma
 * frame — currentStep: null hides it, same pattern /streak and /summary
 * already use) and no MascotBubble (nothing in the frame uses it).
 *
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      {/* Bottom nav — decorative except the path tab (this screen); the
          other 4 icons have no destination in this prototype.
          Deliberately NOT the same box size for all five: measured each
          asset's actual opaque-pixel bounds (canvas readback, not just its
          declared viewBox) — the 4 line icons only fill ~51% of their own
          40x40 canvas, while the avatar badge's circle fills ~99.5% of its
          24x24 canvas edge-to-edge. Equal *boxes* at a small render size
          therefore read as very unequal glyphs; 32px line icons (~16.5px
          effective) and a 16px avatar (~15.9px effective) match instead. */}
      <motion.div
      ...
      >
        ...
        <button type="button" aria-label="Profile" className="flex items-center justify-center p-2">
          <img src="/images/navigation-avatar-icon.svg" alt="" aria-hidden className="h-4 w-4" />
        </button>
      </motion.div>
    </div>
  );
